...
const Carousel = ({ data }) => {
  ...
  const nextSlide = () => {
    setCurrentIndex(
      (prevIndex) => (prevIndex + 1) % Math.ceil(data.length / 4)
    );
  };
  ...
  const prevSlide = () => {
    setCurrentIndex(
      (prevIndex) =>
        (prevIndex - 1 + Math.ceil(data.length / 4)) %
  ...
  return (
    <div className="carousel px-5">
      ...
      <button onClick={prevSlide} className="flex justify-center items-center rounded-full w-14 h-14 bg-[#ff6106] text-white"><MdChevronLeft className=" font-bold" /></button>
        </div>
      <div className=" grid grid-cols-2 overflow-hidden p-10 justify-center items-center">
        {data
      ...
      <div className=" flex justify-center items-center">
      <button onClick={nextSlide} className=" flex justify-center items-center rounded-full w-14 h-14 bg-[#ff6106] text-white"><MdKeyboardArrowRight className=" font-bold text-xl"/></button>
      </div>
    </div>
  );
};
